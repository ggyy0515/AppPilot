---
name: ap-ios-debug-skill
description: Use when diagnosing or operating an AppPilot-integrated iOS Debug build through the stable ap-ios-debug JSON CLI over paired USB or loopback TCP.
---

# AppPilot — ap-ios-debug-skill

Use this skill only for an App whose developer has integrated `APIOSDebugKit`, normally through `APIOSDebugBootstrap`. It is a Debug-build diagnostic surface, not general UI automation.

## Required order

1. Confirm installation with `command -v ap-ios-debug`.
2. Run `ap-ios-debug --json doctor` and resolve every failing prerequisite before operating the App.
3. Run `ap-ios-debug --json devices list`; if a name must be resolved, run `ap-ios-debug --json devices resolve --name '<exact name>'`. Never guess when more than one device is available.
4. Run `ap-ios-debug --json app probe --device "$DEVICE_ID"`.
5. Run `ap-ios-debug --json actions list --device "$DEVICE_ID"` before every action sequence. Prefer `ap-ios-debug --json state get --device "$DEVICE_ID"` or a screenshot for read-only evidence.
...
8. If a high-level command is absent, only use `ap-ios-debug --json request get /v1/capabilities --device "$DEVICE_ID"`. Never construct a raw write request.

Keep JSON stdout machine-readable and treat stderr as diagnostics. Use `set -euo pipefail` in multi-command shell flows; do not let `tee` or another pipeline command mask an `ap-ios-debug` failure. Never print `AP_IOS_DEBUG_TOKEN`, Authorization headers, pairing data, or unrelated App payloads. Do not describe a simulator check as a real-device check.
...
ap-ios-debug --json actions activate "$ACTION_ID" --device "$DEVICE_ID"
...
ap-ios-debug --json state get --device "$DEVICE_ID"
ap-ios-debug --json screenshot capture --device "$DEVICE_ID" --out .ap-ios-debug/artifacts/current.png
test -s .ap-ios-debug/artifacts/current.png
```
...
set -euo pipefail
DEVICE_ID='00008110-EXAMPLE'
OUT='.ap-ios-debug/artifacts/diagnostic.mp4'
mkdir -p "$(dirname "$OUT")"
...
- `auth_required` or `auth_failed`: ask the user to set or correct `AP_IOS_DEBUG_TOKEN` without showing its value.
