...
@MainActor public final class APIOSDebugRuntime {
    ...
    struct Limits: Sendable {
        ...
        static let production = Limits(
            requestBodyBytes: APIOSDebugProtocol.maximumRequestBodyBytes,
            pngBytes: APIOSDebugProtocol.maximumPNGBytes,
            mp4Bytes: APIOSDebugProtocol.maximumMP4Bytes
        )
    }
...
private struct RouteContext: Sendable {
    ...
    let screenshot: APIOSDebugRuntime.ScreenshotSource
    let recording: APIOSDebugRuntime.RecordingSource
    let limits: APIOSDebugRuntime.Limits
}
...
private enum RouteRegistrar {
    static func register(on router: HTTPRouter, context: RouteContext) async {
        await registerGET(on: router, pattern: "/v1/health") { _, _, requestID in
            var data: [String: JSONValue] = [
                "protocol_version": .number(Double(APIOSDebugProtocol.version)),
        ...
        await registerGET(on: router, pattern: "/v1/capabilities") { _, _, requestID in
            ...
            return success(
                .object([
                    ...
                    "limits": .object([
                        "header_bytes": .number(Double(APIOSDebugProtocol.maximumHeaderBytes)),
                        "request_body_bytes": .number(Double(APIOSDebugProtocol.maximumRequestBodyBytes)),
                        "state_bytes": .number(Double(APIOSDebugProtocol.maximumStateBytes)),
                        "png_bytes": .number(Double(APIOSDebugProtocol.maximumPNGBytes)),
                        "mp4_bytes": .number(Double(APIOSDebugProtocol.maximumMP4Bytes)),
    ...
    private static func binary(
        ...
        headers["X-IOS-Debug-Protocol-Version"] = String(APIOSDebugProtocol.version)
