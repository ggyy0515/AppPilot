...
@Suite struct PublicContractTests {
    ...
    @Test func protocolConstantsAndOwnedErrorCodesStayFrozen() {
        ...
        #expect(APIOSDebugProtocol.defaultPort == 9_876)
        #expect(APIOSDebugProtocol.maximumHeaderBytes == 32 * 1_024)
        #expect(APIOSDebugProtocol.maximumRequestBodyBytes == 1 * 1_024 * 1_024)
        #expect(APIOSDebugProtocol.maximumStateBytes == 4 * 1_024 * 1_024)
        #expect(APIOSDebugProtocol.maximumPNGBytes == 25 * 1_024 * 1_024)
        #expect(APIOSDebugProtocol.maximumMP4Bytes == 500 * 1_024 * 1_024)
    ...
    @Test func everyCorePublicSignatureAndRouteCompiles() async throws {
        ...
        var parser = HTTPRequestParser(
            maximumHeaderBytes: APIOSDebugProtocol.maximumHeaderBytes,
            maximumBodyBytes: APIOSDebugProtocol.maximumRequestBodyBytes
        )
        ...
        #expect(parser.maximumHeaderBytes == APIOSDebugProtocol.maximumHeaderBytes)
        #expect(parser.maximumBodyBytes == APIOSDebugProtocol.maximumRequestBodyBytes)
...
@MainActor private func compileAPIOSDebugRuntimePublicContract() async throws {
    let configuration = try APIOSDebugRuntime.Configuration(
        port: APIOSDebugProtocol.defaultPort,
        bearerToken: nil,
    ...
    let runtime = APIOSDebugRuntime(configuration: configuration, stateProvider: provider, actionRegistry: registry)
    _ = runtime.actions
    try await runtime.start()
    await runtime.stop()

    let router = HTTPRouter(authenticator: BearerAuthenticator(token: nil))
    let server = try NetworkDebugServer(port: APIOSDebugProtocol.defaultPort, router: router)
