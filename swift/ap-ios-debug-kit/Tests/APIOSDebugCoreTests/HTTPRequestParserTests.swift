...
@Test func enforcesHeaderByteLimit() throws {
    ...
    let acceptedWire =
        "GET /v1/health HTTP/1.1\r\nX-Padding: \(String(repeating: "a", count: APIOSDebugProtocol.maximumHeaderBytes - fixed.utf8.count))\r\n\r\n"
    #expect(acceptedWire.utf8.count == APIOSDebugProtocol.maximumHeaderBytes)
    var accepted = HTTPRequestParser()
    #expect(try accepted.append(Data(acceptedWire.utf8))?.path == "/v1/health")

    let rejectedWire =
        "GET /v1/health HTTP/1.1\r\nX-Padding: \(String(repeating: "a", count: APIOSDebugProtocol.maximumHeaderBytes - fixed.utf8.count + 1))\r\n\r\n"
...
@Test func enforcesBodyByteLimit() throws {
    let limit = APIOSDebugProtocol.maximumRequestBodyBytes
    var accepted = HTTPRequestParser()
