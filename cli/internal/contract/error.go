...
var descriptors = map[Code]descriptor{
	...
	NoDevice:                   {"No available iOS device was found.", "Connect and unlock one physical iOS device, then run ap-ios-debug --json devices list.", 3},
	...
	ProtocolMismatch:           {"The App debug protocol is incompatible.", "Use an APIOSDebugKit build that implements protocol version 1.", 5},
	AuthRequired:               {"The App requires a bearer token.", "Set AP_IOS_DEBUG_TOKEN in this shell and retry.", 5},
	AuthFailed:                 {"The bearer token was rejected.", "Replace AP_IOS_DEBUG_TOKEN with the token configured by the Debug App.", 5},
	ActionNotFound:             {"The action is no longer registered.", "Run ap-ios-debug --json actions list again and use a current identifier.", 5},
	...
	RecordingInvalidState:      {"The recording command is invalid for the current state.", "Run ap-ios-debug --json recording status before retrying.", 5},
