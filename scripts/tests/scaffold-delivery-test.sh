...
work="$(mktemp -d "${TMPDIR:-/tmp}/ios-debug-scaffold-delivery-test.XXXXXX")"
work="$(cd "$work" && pwd -P)"
...
make -C "$fixture" BUILD_DIR="$work/external" clean
test ! -e "$fixture/build"
test -f "$work/external/sentinel"

fake="$work/failing-ios-debug"
cat >"$fake" <<'EOF'
#!/usr/bin/env bash
printf '{"ok":false,"error":"captured-json"}\n'
printf 'captured-stderr token=%s\n' "${IOS_DEBUG_TOKEN-unset}" >&2
exit 9
EOF
chmod +x "$fake"
set +e
diagnostic="$(IOS_DEBUG_BIN="$fake" IOS_DEBUG_TOKEN='must-not-leak' "$root/scripts/scaffold-smoke.sh" 2>&1)"
status=$?
set -e
test "$status" -ne 0
...
if grep -Fq 'must-not-leak' <<<"$diagnostic"; then
  echo "scaffold smoke leaked IOS_DEBUG_TOKEN" >&2
  exit 1
fi
