...
if [[ "${AP_IOS_DEBUG_REAL_DEVICE_SMOKE:-0}" != "1" ]]; then
  echo "SKIP: device-smoke requires AP_IOS_DEBUG_REAL_DEVICE_SMOKE=1"
  exit 0
fi
...
device="${AP_IOS_DEBUG_DEVICE:-}"
team="${AP_IOS_DEBUG_DEVELOPMENT_TEAM:-}"

if [[ -z "$device" ]]; then
  echo "SKIP: device-smoke requires AP_IOS_DEBUG_DEVICE for an explicit trusted device"
  exit 0
fi
if [[ -z "$team" ]]; then
  echo "SKIP: device-smoke requires AP_IOS_DEBUG_DEVELOPMENT_TEAM for signing"
  exit 0
fi
...
cleanup() {
  ...
  if [[ "$recording_active" -eq 1 && -n "$device_id" ]]; then
    ...
    AP_IOS_DEBUG_TOKEN="$token" "${cli[@]}" recording status \
      >"$tmp/cleanup-recording-status.json" 2>"$tmp/cleanup-recording-status.json.stderr" || true
    AP_IOS_DEBUG_TOKEN="$token" "${cli[@]}" recording stop --out "$cleanup_mp4" \
...
test -x "$binary" || { failure_stage="locate ap-ios-debug CLI"; exit 1; }
...
if ! run_capture "resolve and check device readiness" "$resolve" \
  env AP_IOS_DEBUG_TOKEN="$token" "$binary" --json devices resolve --name "$device"; then
...
# because this invocation has not installed or launched APIOSDebugDemo yet.
preflight="$tmp/device-readiness.json"
if ! run_capture "check selected device trust and lock readiness" "$preflight" \
  env AP_IOS_DEBUG_TOKEN="$token" "${cli[@]}" app probe; then
...
if ! run_capture "build and sign APIOSDebugDemo" "$xcode_log" \
  "$xcodebuild_bin" -project "$root/Examples/ap-ios-debug-demo/ap-ios-debug-demo.xcodeproj" \
    -scheme ap-ios-debug-demo -configuration Debug -destination "id=$device_id" \
...
app="$derived/Build/Products/Debug-iphoneos/APIOSDebugDemo.app"
run_capture "validate signed APIOSDebugDemo build product" "$tmp/app-check.txt" test -d "$app"
run_capture "install signed APIOSDebugDemo" "$tmp/install.log" \
  "$xcrun_bin" devicectl device install app --device "$device_id" "$app" \
    --json-output "$tmp/install.json"
run_capture "launch APIOSDebugDemo with authentication" "$tmp/launch.log" \
  env DEVICECTL_CHILD_AP_IOS_DEBUG_TOKEN="$token" \
...
if [[ ! "$launched_pid" =~ ^[0-9]+$ || "$launched_pid" -eq 0 ]]; then
  failure_stage="read launched APIOSDebugDemo process identifier"
...
set +e
env -u AP_IOS_DEBUG_TOKEN "${cli[@]}" actions list >"$tmp/auth-missing.json" 2>"$tmp/auth-missing.json.stderr"
missing_status=$?
env AP_IOS_DEBUG_TOKEN="$wrong_token" "${cli[@]}" actions list >"$tmp/auth-wrong.json" 2>"$tmp/auth-wrong.json.stderr"
wrong_status=$?
set -e
...
run_capture "start short recording" "$tmp/recording-start.json" \
  env AP_IOS_DEBUG_TOKEN="$token" "${cli[@]}" recording start
...
run_capture "read active recording status" "$tmp/recording-status.json" \
  env AP_IOS_DEBUG_TOKEN="$token" "${cli[@]}" recording status
...
run_capture "stop, download, verify, and delete recording" "$tmp/recording-stop.json" \
  env AP_IOS_DEBUG_TOKEN="$token" "${cli[@]}" recording stop --out "$mp4"
recording_active=0
